...
class ProgramParser:
    ...
    _kernel_def_pattern:re.Pattern = re.compile(r"^\s*__global__\s+void\s+(?P<name>[a-zA-Z_]\w*)\s*\((?P<body>.*?)\)", re.MULTILINE | re.DOTALL)
    _type_qualifier_pattern:re.Pattern = re.compile(r"\b(const|__restrict__|volatile)\b")
    ...
    def __init__(self):
        ...
        self._options_ptr:ptr_ptr_char = None
        self._n_options:int = 0
    ...
    def parse(self, file_name:str, includes:Optional[List[str]]=None, defines:Optional[Dict[str, Any]]=None, options:Optional[BuildOptions]=None)->Union[float, bool]:
        ...
        self._cache_folder:str = os.path.dirname(os.path.abspath(file_name)).replace("\\", "/") + "/__cucache__"
        ...
        self._options:BuildOptions = copy.deepcopy(options) if options is not None else BuildOptions()
        options_list:List[str] = self._options.to_list()

        self._n_options:int = len(options_list)
        self._options_ptr:ptr_ptr_char = (c_char_p * self._n_options)(*[option.encode("utf-8") + b"\0" for option in options_list])
    ...
    def generate_pyi(self)->None:
        pyi_file_name = self._file_name.replace(".cu", ".pyi")
...
"""
    ...
    @property
    def options_ptr(self)->ptr_ptr_char:
    ...
    @staticmethod
    def _apply_structure_pointers(value, cmd_queue):
    ...
    def _create_struct_type(self, struct_info:StructInfo)->type:
        ...
        for member in struct_info.members.values():
            if member.base_type_str in CUInfo.basic_types:
                base_type = CUInfo.basic_types[member.base_type_str]
            else:
        ...
        def apply_pointers(self, cmd_queue)->List[Dict[str, Any]]:
            result = []
            for field in self._fields_:
                if issubclass(field[1], _Pointer):
                    ori_name:str = field[0].replace(f"_{self.__class__.__name__}_ptr_", "")
        ...
        type_body = {
            ...
            "_fields_": _fields_,
            "dtype": dtype,
            "_members": struct_info.members,
            "_pointer_types": pointer_types,
    ...
    def _find_type_qualifiers(self, arg_code:str)->List[str]:
        ...
        for match in matches:
            ...
            type_qualifiers.append(match_str.upper())
