...
class image1d_array(imagend):

    def __init__(self, context:Context, image:image1d_array_t):
    ...
    def write(self, cmd_queue:CommandQueue, origin:Tuple[int, int]=(0, 0), region:Optional[Tuple[int, int]]=None, host_ptr:c_void_p=None, after_events:List[Event]=None)->Event:
        if cmd_queue.context != self.context:
            raise ValueError("cmd_queue should be in the same context as current image1d_array")
        ...
        ptr_origin = (c_size_t * 3)(origin[1], 0, origin[0])
        ptr_region = (c_size_t * 3)(region[1], 0, region[0])
